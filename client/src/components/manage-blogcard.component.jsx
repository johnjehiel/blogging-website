...
const BlogStats = ({ stats }) => {
    return (
        <div className="flex gap-2 max-lg:mb-6 max-lg:pb-6 border-grey max-lg:border-b">
            {
                Object.keys(stats).map((infoKey, i) => {
                    return !infoKey.includes("parent") ? <div key={i} className={"flex flex-col items-center w-full h-full justify-center p-4 px-6 " + (i != 0 ? "border-grey border-l " : "")}>
                        <h1 className="text-xl lg:text-2xl mb-2">{stats[infoKey].toLocaleString()}</h1>
                        <p className="max-lg:text-dark-grey capitalize">{infoKey.split("_")[1]}</p>
                    </div> : ""
                })
            }
        </div>
    ...
    return (
        <>
            <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
                ...
                <img src={banner}
                     alt="banner"
                     className="max-md:hidden lg:hidden xl:block w-28 h-28 rounded-lg flex-none bg-grey object-cover"
                />
                ...
                <div className="flex flex-col justify-between py-2 w-full min-w-[300px]">
                    ...
                    <div className="flex gap-6 mt-3">
                        ...
                        <button className="pr-4 py-2 underline text-red" onClick={() => deleteBlog(blog, access_token, e.target)}>Delete</button>
                    </div>
                </div>
    ...
    return (
        <div className="flex gap-5 lg:gap-10 pb-6 border-b mb-6 border-grey">
            ...
            <div>
                ...
                <p className={"line-clamp-2 font-gelasio " + (!des.length ? "text-dark-grey" : "") }>{des.length ? des : "No Description"}</p>
...
const deleteBlog = (blog, access_token, target) => {
    ...
    .then(({ data }) => {
        ...
        setStateFunc(preVal => {
            ...
            /* when there is nothing in the results array then the use effect will automatically
            get more blogs so we dont need to implement anything here and we can directly return from the deleteBlog function*/
