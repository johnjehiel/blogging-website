...
export const fetchComments = async ({ skip = 0, blog_id, setParentCommentCountFun, comment_array = null }) => {
    ...
    .then(({ data }) => {
        ...
        data.map(comment => {
            comment.childrenLevel = 0; // add childrenLevel as 0 to all comments
        })
...
const CommentsContainer = () => {
    ...
    const loadMoreComments = async () => {
        ...
        let newCommentsArr = await fetchComments({ skip: totalParentCommentsLoaded, blog_id: _id, setParentCommentCountFun: setTotalParentCommentsLoaded, comment_array: commentsArr }); // get some more comments

        setBlog({ ...blog, comments: newCommentsArr }); // set the newly loaded comments
