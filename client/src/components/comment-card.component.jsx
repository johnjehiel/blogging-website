...
const CommentCard = ({ index, leftVal, commentData }) => {
    ...
    const LoadMoreRepliesButton = () => {
        ...
        let button = <button className="text-dark-grey p-2 px-3 hover:bg-grey/70 rounded-md flex items-center gap-2"
                            onClick={() => loadReplies({ skip: index - parentIndex, currentIndex: parentIndex })} // to skip all the already rendered comments and this will make the functionality to behave as if we have clicked the loadreplies of the parent comment
                            >Load More Button</button>

        // IMPORTANT: we are finding a seperation between the parent comment and children comment so that we can render the load more button at the very last children comment
        if (commentsArr[index + 1]) {
            if (commentsArr[index + 1].childrenLevel < commentsArr[index].childrenLevel) { // making sure that the childrens level is greater than the parent level to render the load more button
            ...
            <div className="my-5 p-6 rounded-md border border-grey">
                ...
                <div className="flex gap-5 items-center mt-5">

                    {
                        ...
                        <button className="text-dark-grey p-2 px-3 hover-bg-grey/30 rounded-md flex items-center" onClick={hideReplies}>
